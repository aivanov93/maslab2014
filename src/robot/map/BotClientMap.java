...
import robot.map.BotClientMap.Wall.WallType;
...
public class BotClientMap {
	...
	public ArrayList<LineSegment2D> getMaze(){
		ArrayList<LineSegment2D> maze=new ArrayList<LineSegment2D>();
		for (Wall wall:walls){
			maze.add(new LineSegment2D(wall.start.x*gridSize, wall.start.y*gridSize, wall.end.x*gridSize, wall.end.y*gridSize));
		}
		return maze;
	}
	
	public ArrayList<LineSegment2D> getReactors(){
		ArrayList<LineSegment2D> reactors=new ArrayList<LineSegment2D>();
		for (Wall wall:walls){
			if (wall.type==WallType.REACTOR){
			reactors.add(new LineSegment2D(wall.start.x*gridSize, wall.start.y*gridSize, wall.end.x*gridSize, wall.end.y*gridSize));
		}}
		return reactors;
	}
	
	public ArrayList<LineSegment2D> getYellowWalls(){
		ArrayList<LineSegment2D> yellow=new ArrayList<LineSegment2D>();
		for (Wall wall:walls){
			if (wall.type==WallType.OPPONENT){
			yellow.add(new LineSegment2D(wall.start.x*gridSize, wall.start.y*gridSize, wall.end.x*gridSize, wall.end.y*gridSize));
		}}
		return yellow;
	}
	
	public LineSegment2D getSilo(){
		for (Wall wall:walls){
			if (wall.type==WallType.SILO){
			return new LineSegment2D(wall.start.x*gridSize, wall.start.y*gridSize, wall.end.x*gridSize, wall.end.y*gridSize);
		}}
		return new LineSegment2D(0,0,0,0);
	}
	
	public Position getPosition(){
		return new Position(startPose.x*gridSize, startPose.y*gridSize, startPose.theta, 1);
	}
